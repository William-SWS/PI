...
def main() -> None:  # pipeline: carrega -> threshold -> fechamento -> dist transform -> markers -> watershed
    ...
    image_path = IMG_DIR / "3_pilha_roupas.png"
    ...
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_grayscale.png"), gray_image)
    log.info("  Grayscale salva (dimensões: %s)", gray_image.shape)

    binary_image = binarize(gray_image)
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_threshold.png"), binary_image)
    ...
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_closing.png"), closed_image)
    ...
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_distance_transform.png"), distance_visualization)
    log.info("  Distance transform calculado (chamfer 3-4-5)")

    marker_mask = find_markers_from_distance(distance_image)
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_markers.png"), marker_mask)
    ...
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_markers_overlay.png"), markers_overlay)
    ...
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_boundaries.png"), boundary_map)
    log.info("  Linhas de watershed detectadas")

    segmented_gray = normalize_to_uint8(label_map.astype(np.float64))
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_segmented_gray.png"), segmented_gray)
    log.info("  Segmentação em tons de cinza salva")

    segmented_color = apply_pseudo_colors(label_map)
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_segmented_color.png"), segmented_color)
    ...
    cv2.imwrite(str(OUT_DIR / "3_pilha_roupas_segmented_output.png"), boundaries_on_original)
